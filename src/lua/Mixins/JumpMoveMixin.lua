...
local kMinLandIntensitySpeed = 7
local kMaxLandIntensitySpeed = 15
...
function JumpMoveMixin:__initmixin()
    ...
    PROFILE("JumpMoveMixin:__initmixin")
...
function JumpMoveMixin:ModifyVelocity(input, velocity, deltaTime)
    ...
    if not self.jumpHandled and bit_band(input.commands, Move.Jump) ~= 0 then
...
function JumpMoveMixin:OnGroundChanged(onGround, intensity)
    
    if onGround then
        
        if intensity then
            local weapon = self:GetActiveWeapon()
            if weapon and weapon.UseLandIntensity and weapon:UseLandIntensity() then 
                self.landIntensity = Clamp( intensity / kMaxLandIntensitySpeed, 0, 1 )
            end
        end
...
function JumpMoveMixin:OnUpdateAnimationInput(modelMixin)
    ...
    if self:GetIsJumping() and (not HasMixin(self, "LadderMove") or not self:GetIsOnLadder()) and (not self.GetHasMetabolizeAnimationDelay or not self:GetHasMetabolizeAnimationDelay()) then
        ...
        elseif self:isa("JetpackMarine") then
            if not ( self:GetIsJetpacking() or self:FallingAfterJetpacking() ) then
                modelMixin:SetAnimationInput("move", "jump")
            end
        else
            modelMixin:SetAnimationInput("move", "jump")
