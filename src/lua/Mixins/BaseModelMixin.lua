...
BaseModelMixin.optionalConstants = {
    ...
    kTriggeringEnabledDefault = "Enable/Disable Triggering when setting the model. Does nothing if nil"
}
...
local function UpdateAnimationState(self, allowedOnClient, transition)
    ...
    if allowed and model ~= nil and graph ~= nil then
        ...
        self.passedTags:Clear()
        state:Update2(graph, model, self.poseParams, prevTime, time, self.passedTags)

        if transition then

            local self_OnTag = self.OnTag
            if self_OnTag then
                for i = 1, self.passedTags:GetCount() do
                    local tagIndex = self.passedTags:Get(i)
            ...
            if self.passedTags:GetCount() > 0 then
                UpdateAnimationInput(self, state, graph)
            end
            ...
            if allowedOnClient or Server then
                ...
                self.passedTags:Clear()
                state:Transition2(graph, model, self.passedTags)
...
local function SetHighlight(self)
    ...
    if self:GetIsHighlightEnabled() ~= nil then
        ...
        if HasMixin(self, "Team") and self:GetTeamNumber() == kMarineTeamType and (self:isa("Player") or self:isa("Mine") or self:isa("Grenade") or self:isa("Sentry") or self:isa("ClusterGrenadeThrower") or self:isa("GasGrenadeThrower") or self:isa("PulseGrenadeThrower")) then
            self._renderModel:SetMaterialParameter("highlight", 1)
        elseif HasMixin(self, "Team") and self:GetTeamNumber() == kMarineTeamType then
            self._renderModel:SetMaterialParameter("highlight", 0.98)
        elseif HasMixin(self, "Team") and self:GetTeamNumber() == kAlienTeamType and self:isa("Player") and not self:isa("Gorge") then
            self._renderModel:SetMaterialParameter("highlight", 0.96)
        elseif self:isa("Gorge") or self:isa("Babbler") then
            self._renderModel:SetMaterialParameter("highlight", 0.94)
        else
            self._renderModel:SetMaterialParameter("highlight", 0.90)
        end
    end
end
...
local function UpdateRenderModel(self)
    ...
    if self._updateHighlight and self._renderModel then
        self._updateHighlight = false
        SetHighlight(self)
    end
...
function BaseModelMixin:SetHighlightNeedsUpdate()
    self._updateHighlight = true
end
...
function BaseModelMixin:__initmixin()
    ...
    PROFILE("BaseModelMixin:__initmixin")
...
-- Since predict world doesn't perform regular entity updates (OnUpdate()), we need to setup
-- listeners for fields that necessitate a change.  Ie, changing the model index necessitates
-- a change in the model, but this normally won't happen on Predict unless we explicitly
-- listen for it.
local function OnPredictModelChange(self)

    if self.previousModelIndex ~= self.modelIndex then
        self.previousModelIndex = self.modelIndex
...
function BaseModelMixin:OnInitialized()
    ...
    if Predict then
        self:AddFieldWatcher("modelIndex", OnPredictModelChange)
    end
end
...
function BaseModelMixin:SetModelMass(mass)
    assert(type(mass) == "number" and mass > 0.0)
    local model = Shared_GetModel(self.modelIndex)
    if model then
        model:SetModelMass(mass)
    end
end

function BaseModelMixin:GetModelMass()
    local model = Shared_GetModel(self.modelIndex)
    if model then
        return model:GetModelMass()
    end
    return nil
end
...
function BaseModelMixin:UpdatePhysicsModel()
    ...
    if (self.physicsModelIndex ~= self.modelIndex) and self:GetPhysicsModelAllowed() then
        ...
        if self.physicsModel ~= nil then
            ...
            if self:GetMixinConstants().kTriggeringEnabledDefault then
                self.physicsModel:SetTriggeringEnabled(self:GetMixinConstants().kTriggeringEnabledDefault)
            end
    ...
    if self.physicsModel ~= nil then
        ...
        self.physicsModel:SetCollisionEnabled(self:GetIsVisible())      --TODO Review/revise, this could likely be a large cause of PredictedProjectile wonkiness
...
local OnCommandAnimationEnable
local OnCommandPoseParamsEnable
